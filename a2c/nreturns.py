...
entropy_const = 0.005 # Scales the entropy portion of the loss function
max_norm = 0.3 # Scales the gradients using their norm
...
net_save_file = "nrets_net_state_dict.p"
optim_save_file = "nrets_optim_state_dict.p"
...
while T < max_tsteps:
        ...
        for t in range(n_tsteps):
            ...
            if done or t==n_tsteps-1 or rewards[-1] != 0: # Reached end of rollout for this episode
                ...
                if rewards[-1] == 0: # Did not reach a terminal state (pong specific)
                    rewards[-1] = values[-1] # Set bootstrapped reward for fitting critic later
                    advantages.append(0) # use bootstrapped advantage r[-1]-v[-1]
    ...
    returns = Variable(advantages + values.data.squeeze())
    advantages = Variable(advantages)
    #advantages = Variable((advantages-torch.mean(advantages))/(torch.std(advantages)+1e-7))
    #returns = Variable((returns - torch.mean(returns))/(torch.std(returns)+1e-7))

    action_loss = -torch.mean(action_logs*advantages) # Standard policy gradient
    value_loss = val_const*mseloss(values.squeeze(), returns)
